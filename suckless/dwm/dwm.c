...
#define MAXSIDE 3 /* max windows per side */
...
struct Client { /* a window that dwm is managing */
	...
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, issticky, isterminal, noswallow; 
		// isfakefullscreen; /* window states */
	int fakefullscreen;
...
{
	int x, w, wdelta, tw = 0; /* x pos, width, and text width */
...
{ /* handling when focus shifts to a different client */
	...
	if (c) {
		 ...
		 * is restored */
		if (selmon->lt[selmon->sellt]->arrange != centeredmonocle) {
		    if (!solitary(c))
				XSetWindowBorder(dpy, c->win, scheme[SchemeSel][ColBorder].pixel);
		}
...
opacity(Client *c, double opacity)
{
        if(opacity > 0 && opacity < 1) {
				...
				XChangeProperty(dpy, c->win, XInternAtom(dpy, "_NET_WM_WINDOW_OPACITY", False),
                                XA_CARDINAL,32, PropModeReplace, (unsigned char *)real_opacity,
                                1);
        } else
                XDeleteProperty(dpy, c->win, XInternAtom(dpy,"_NET_WM_WINDOW_OPACITY", False));
}
...
resizemouse(const Arg *arg)
{
	...
	if (!(c = selmon->sel))
		return;
    if (c->isfullscreen
    &&  c->fakefullscreen !=1) /* no support resizing fullscreen windows by mouse */
		return;
	restack(selmon);
	ocx = c->x;
	ocy = c->y;
...
centeredmonocle(Monitor *m) {
    ...
    float factor = m->mfact;

    int mw = m->ww * factor;
    int mx = m->wx + (m->ww - mw) / 2;
    int my = m->wy;
    int mh = m->wh;
...
togglefakefullscreen(const Arg *arg)
{
	...
	c->fakefullscreen = !c->fakefullscreen;
